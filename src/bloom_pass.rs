...
impl BlurPass {
    pub fn new(device: &wgpu::Device, frame_buffers: &FrameBuffers, samplers: &Samplers) -> Self {
        ...
        let blur_bind_groups = std::iter::once(&frame_buffers.bright_texture_view)
            .chain({
                let len = frame_buffers.bloom_blur_buffers.len();
                frame_buffers.bloom_blur_buffers[0..len - 1]
                    .into_iter()
                    .map(|buf| &buf.texture_view)
            })
            .map(|texture_view| {
    ...
    pub fn recreate_bind_group(
        ...
        self.blur_bind_groups = std::iter::once(&frame_buffers.bright_texture_view)
            .chain({
                let len = frame_buffers.bloom_blur_buffers.len();
                frame_buffers.bloom_blur_buffers[0..len - 1]
                    .into_iter()
    ...
    pub fn draw(&self, encoder: &mut wgpu::CommandEncoder, frame_buffers: &FrameBuffers) {
        let attachment_views = (&frame_buffers.bloom_blur_buffers)
            .into_iter()
            .map(|buf| &buf.texture_view);
        for (attachment_view, bind_group) in
